...
. setup.sh
```
The automated setup will run, insLink to PreCDC Scoreboardtalling necessary files and setting up the launcher. Go to the desktop and double-click on the 'Run Slot Machine' icon, and the 'Scoreboard' icon. If the file opens in gedit, right click on the file and click "enable launching". The launcher should now run the slot machine application.
